...
## 2. 情報アーキテクチャ（2層コンテンツ + AI Chat 深度級のみ）

> 概要級は 2026-03-03 に全面廃止。Guest は AI Chat 利用不可（403）。
...
┌────────────────────▼─────────────────────────────┐
│  AI Chat（深度級のみ）                              │
│  ✅ Profile 個性化 + AI-only 全量 + web_search      │
│  ✅ 具体機関 / 決策樹 / 地域差 / 跨域連動            │
│  ※ "あなたはどうすべきか"（核心壁壘）                │
│  ※ Guest → 403（登録必須）                          │
...
> 概要級は 2026-03-03 に廃止。Guest は AI Chat 利用不可（403 CHAT_REQUIRES_AUTH）。
> 登録 CTA を表示し、Free アカウント作成を促す。
