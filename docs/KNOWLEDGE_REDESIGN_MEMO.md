...
```
Agent の知識根源 = モデル base 知識 + knowledge/（経験・暗黙知・判断ロジック含む）
guides/ の生成ソース = モデル base 知識 + knowledge/（最核心の経験・判断ロジックを除く）
```

- **Agent が回答に使う知識** = モデルが元々知っていること + knowledge/ の全内容 + web search
- **guides/ に書く知識** = 上記から最核心の経験と判断ロジックを除いたもの
- **guides/ に書かない核心知識** = AI 向導（Layer 3）でのみユーザーに提供 → 差別化壁壘
...
### Phase 1: アーキテクチャ変更（knowledge/guides 分離）— ✅ 完了

- [x] guides/ ディレクトリ新設、Navigator API 参照先変更 (``73c098d``)
- [x] 全 svc-* AGENTS.md 書き換え（web_search 活用ルール含む）
- [x] Frontend ロック表示 + 升級 CTA
- [x] 6 Agent 移行完了

### Phase 2: Knowledge 再構築（← 現在ここ）

各ドメインの knowledge/ を「モデルが知らない + 検索でも出てこない情報」に刷新。

- [ ] **svc-finance**: Z の金融実践経験をヒアリング → knowledge 再構築
- [ ] **svc-tax**: 税務 knowledge 作成
- [ ] **svc-visa**: 既存 knowledge の精査 + 経験追加
- [ ] **svc-medical**: 既存 knowledge の精査 + 経験追加
- [ ] **svc-life**: 既存 knowledge の精査 + 経験追加
- [ ] **svc-legal**: knowledge 新規作成
- [ ] 旧 knowledge ファイル（一般知識の繰り返し部分）の削除

### Phase 3: 指南生成
- [ ] 生成ルール定義（free/premium 境界 per ドメイン）
- [ ] knowledge → guides 変換プロンプト作成
- [ ] AI で free/premium 指南を生成 → Z レビュー → 確定
- [ ] 多言語版生成（EN/ZH + 将来 KO/VI/PT）
...
### 2026-02-20 21:30~ Agent 体系再設計
- Z: agent の粒度はどうやって決める？同一 agent にまとめる基準は？knowledge なら skill で動的に読めるのでは？
- OC: 分析の結果、agent 分離の真の価値は knowledge ではなく **System Prompt（思維モード）** にある
  - 金融 = コスト/リスク/収益分析
  - 法律 = 処境分析 + 求助先誘導
  - 生活 = how-to、手順案内
  → **「異なる思維モードが必要か？」が分離の基準**
- Z: life が広すぎる。法律を分離すべきでは？
  - OC 同意: 法律の回答モードは life とは異なる（「やり方を教える」vs「処境を分析して専門家に誘導」）
- Z: 跨領域問題（例: 妊娠出産 = medical + life + tax + visa + legal）はどうする？
  - OC 提案: 現段階は **方案 A（主 agent 回答 + 他領域への案内）**、将来 C に演進
- Z: 士業独占業務の法的制約は？AI が回答してはいけない範囲は？
  - OC 調査: **税理士法52条が最も危険 — 無償でも個別税務相談は違法**
  - 弁護士法72条: 個別法律事件の法的判断は非弁行為
  - 行政書士法19条: 書類作成代行は違法
  - → 全 agent の AGENTS.md に法的制約ルールを明記する必要あり

### 確定: 6 Agent 体系

```
svc-finance  — 金融（銀行/投資/信用卡/保険/贷款/送金）
svc-tax      — 税務（税金/年金/社保/確定申告/ふるさと納税）
svc-visa     — 签证（在留資格/入管/永住/家族）
svc-medical  — 医疗（看病/保険/急救/薬局/心理/妊娠出産の医療面）
svc-life     — 生活（住居/交通/工作/行政/购物/文化/教育）
svc-legal    — 法律（労働紛争/事故/犯罪被害/消費者保護/離婚/権利）
```

+ 路由分類器（Concierge を廃止、軽量 LLM 呼び出しで分類）

### 各 Agent の法的制約（士業独占業務）

#### 重要な判例・先例

**国税庁自身がチャットボット「ふたば」を運営** — 確定申告に関する一般的な Q&A を提供。
→ 公開情報に基づく制度説明・手続き案内・該当判断は適法の証左。

**法務省ガイドライン（令和5年8月）**: AI 契約書レビューについて、
一般的な場面を想定した支援で、個別事案の法的処理でなければ弁護士法72条に違反しない。
...
| Agent | リスク | 根拠法 | 境界線 |
|-------|--------|--------|--------|
| svc-tax | **高** | 税理士法52条 | 公開制度の説明・該当判断・記入ガイドは OK。個別節税戦略は NG。「税理士に要相談」の誘導を常に含める |
| svc-legal | **高** | 弁護士法72条 | 制度説明・事例紹介・権利案内は OK。個別事件への法的判断は NG。「弁護士/行政書士に要相談」を常に含める |
| svc-visa | **中** | 行政書士法19条 | 手続き説明・書類案内は OK。書類作成代行は NG。複雑ケースは「行政書士に依頼を」 |
| svc-finance | **低〜中** | 金商法 | 制度説明・商品比較は OK。個別投資判断は NG |
| svc-medical | **低** | 医師法17条 | 一般健康情報は OK。診断・処方は NG |
| svc-life | **低** | — | ほぼ制約なし |

#### svc-tax の価値定位（法的制約の中で何が提供できるか）

国税庁ふたばとの差異化:
1. **多言語対応** — ふたばは日本語のみ。税務の日本語は外国人に最も難解
2. **プロフィール連動の該当判断** — 在留資格・来日年数・就労形態で「あなたに該当する制度」を案内
3. **平易な言語での解説** — 公開情報を外国人向けに翻訳
4. **書類記入サポート** — 公開記入例を元に「ここにはこの数字を書く」を案内
5. **クロスドメイン連携** — 税金×ビザ（永住申請に納税証明）、税金×金融（口座振替設定）

### 跨領域対応方針

**現段階**: 方案 A — 主 agent が回答 + 他領域への案内テンプレ
**将来**: 方案 C — 各 agent の knowledge に跨領域シナリオ別の知識を配置

例: 妊娠出産
- svc-medical: 産検、母子手帳、出産準備
- svc-tax: 医療費控除、扶養控除、児童手当 → 「一般的な制度として」の範囲で
- svc-visa: 子の在留資格取得（30日以内）
- svc-legal: 育休・産休の権利 → 「一般的に保障されている権利として」
- svc-life: 出生届、保育園
