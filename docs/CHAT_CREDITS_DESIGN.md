...
### 3.2 消費フロー（`check_and_consume()`）

```
1. tier == 'guest' → 403 CHAT_REQUIRES_AUTH
2. tier == 'premium' or 'premium_plus' → 無制限（クレジット不要、直接許可）
3. 有効クレジット行を上記の優先順で 1 行取得
   → 見つかった → remaining -= 1, flush → allowed=True
   → 見つからない → allowed=False, 429 USAGE_LIMIT_EXCEEDED
4. daily_usage.deep_count も +1（analytics 用、判定には使わない）
```
...
| `services/usage.py` | `check_and_increment` → `check_and_consume` にリファクタ。Credit Ledger ベースに | 大改修 |
