...
| Profile 収集 | 登録フォームで必須入力 | trial-setup ダイアログで 3 フィールド収集 |
...
Profile に nationality / residence_status / residence_region が設定済みか確認
  ↓ 未設定
Trial Setup ダイアログを表示
  ├── 国籍（nationality）: プルダウン選択
  ├── 在留資格（residence_status）: プルダウン選択
  └── 居住地域（residence_region）: プルダウン選択
  ↓ 3 フィールド入力完了
...
| 要素 | 理由 |
|------|------|
| アカウント画面の「サブスクリプション」項目 | IAP 未実装のため。表示するとユーザー混乱の原因になる |
