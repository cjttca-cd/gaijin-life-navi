...
- 2026-03-09: ADR-011 追加（LLM Backend Refactor — OpenClaw → Direct API）
...
### ADR-011: LLM Backend Refactor — OpenClaw Subprocess → Direct API (2026-03-09)

**Status**: Approved

**Context**:
AI Chat のレスポンス時間が ~50 秒かかっており、モバイルブラウザでは画面ロック中に HTTP 接続が切断される。原因は OpenClaw CLI subprocess × 2 回（router + domain agent）+ Opus モデルの遅延。

**Decision**:
- OpenClaw subprocess 呼び出しを **ローカル LLM プロキシ（OpenAI 互換 API）への直接 HTTP 呼び出し** に置換
- Router: `gemini-2.5-flash-lite`（~0.6s）、Agent: `gemini-3-flash`（~5-7s）
- Fallback: 両方とも `gemini-2.5-flash`
- Knowledge base は各 domain の workspace/knowledge/ + guides/*.zh.md を全量読み込み、system prompt に注入
- `call_agent()` / `route_to_agent()` の関数シグネチャは **完全に維持**（chat.py 変更なし）

**Consequences**:
- レスポンス時間: **50s → 6-9s**（5-8倍高速化）
- Frontend 変更: **なし**（API contract 不変）
- Knowledge 注入方式が memory_search（セマンティック検索）→ 全量注入に変更。各 domain 50-74KB で Gemini 1M context 内に余裕で収まる
- OpenClaw Gateway は AI Chat 以外の用途（agent 管理、memory cron 等）で引き続き稼働
- Image analysis は一時的に無効化（将来 multi-modal API で対応予定）

**Rollback**: `git checkout v0.9.0-web -- backend/app_service/services/agent.py`

---

### ADR-012: Web Search Integration via Gemini Grounding (2026-03-09)

**Status**: Approved

**Context**:
Knowledge files contain 50+ references to `web_search で確認` — instructions for the agent to search for current fees, rates, deadlines, and contact info. After the LLM refactor (ADR-011), agents lost access to search tools. These instructions are dead code, causing agents to hallucinate outdated numbers or give vague responses.

**Decision**:
- Router outputs JSON `{"domain":"...", "search":"..."}` instead of a single domain word
- Router decides if the query needs web search (current fees, deadlines, policy changes → search; general knowledge → no search)
- Search uses **Gemini Direct API with Google Search Grounding** (`gemini-2.5-flash-lite`, ~4s)
- Search results injected into the agent's user message as `【ウェブ検索結果】` section
- Knowledge files updated: `web_search で確認` → `検索結果を参照`
- Router model stays `gemini-2.5-flash-lite` (verified: can output JSON at 1.2-2.6s)
- Kill switch: `SEARCH_ENABLED=false` disables search without code changes

**Alternatives rejected**:
- DuckDuckGo: 0 results for Japanese queries (tested and confirmed broken)
- Google Custom Search API: 100 queries/day limit
- Function calling (tool_use): Model often ignores the tool; adds extra LLM round-trip
- Always-search: Wastes 4s on queries that don't need it

**Consequences**:
- `route_to_agent()` returns `RoutingResult` (agent_id + search_query) instead of `str` — `routers/chat.py` must adapt (minimal change)
- Latency: no-search queries +1s (JSON overhead), search queries +5s (accurate data vs stale)
- New dependency: `google-genai` package
- New env var: `GEMINI_API_KEY` in systemd service
- Graceful degradation: search failure → proceed without search

**Rollback**: Set `SEARCH_ENABLED=false` or git revert

---

### ADR-013: SSE Streaming for Chat Responses (2026-03-10)

**Status**: Approved

**Context**:
Chat responses take 16-26s. Users see only a loading spinner during this time. The LLM proxy supports SSE streaming, meaning tokens can be delivered incrementally.

**Decision**:
- Add new `/chat/stream` endpoint returning `text/event-stream`
- Keep existing `/chat` endpoint unchanged (backward compat)
- Router + Search remain synchronous; only the agent call is streamed
- SSE events: `routing`, `searching`, `token`, `done`, `error`
- □ tracker items parsed after stream completes (not mid-stream)
- Credit deduction after stream completes (same as sync flow)
- Frontend incrementally renders tokens in the chat bubble

**Consequences**:
- Perceived latency: 16-26s → 4-8s (time to first visible text)
- Total completion time unchanged
- Nginx needs `proxy_buffering off` for `/api/`
- Frontend needs SSE client (Dio stream mode) + incremental rendering
- Both sync and stream endpoints coexist

**Rollback**: Frontend falls back to `/chat` sync endpoint
