...
| S09 | Navigator トップ | `/navigator` | 6 ドメイン一覧（アイコン + 概要キーワード表示）。Coming Soon カテゴリは廃止 | Flutter 全プラットフォーム |
...
- When ガイド一覧画面を表示する
- Then ドメインヘッダー（アイコン + ドメイン名 + ガイド件数）が表示される
- And ガイドがタイトル + サマリー + アクセスバッジ（FREE/🔒）+ タグ（最大3）で表示される
...
- Given ユーザーがガイドを選択した
- When 詳細画面を表示する
- Then ヒーローヘッダー（ドメインバッジ + タイトル + 読了時間 + タグ）が表示される
- And markdown 形式のガイド全文が表示される
- And 同一ドメインの関連ガイド（最大3件）が表示される
